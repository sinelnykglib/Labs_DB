...
from Lab_4 import DB_mong, getAvgSub, addNewStudent, deleteStudent, updateStudent, addTest, updateTest, deleteTest
...
def mg():
    form = testForm(request.form)
    subject_id = form.subject_id.data
    query = f"SELECT subjectname FROM subject WHERE subject_id = {subject_id}"
    result = db.session.execute(query)
    subject_name = result.fetchone()[0] if result.rowcount > 0 else None
    #subject_name = db.session.execute(select(subjectname).where(subject_id=form.subject_id.data))
    saab = ['Ukr','hist','math','phys','chem','bio','geo','eng','fra','deu','spa']
    addTest(student_id=form.student_id.data,
            teststatus=form.teststatus.data,
            ball100=form.ball100.data,
            ball12=form.ball12.data,
            ball=form.ball.data,
            ...
            dpalevel=form.dpalevel.data,
            school_id=form.school_id.data,
            subject_name=subject_name,saab=saab[subject_id-1])
...
def showstatistics():
    ...
    if request.method == 'POST':
        ...
        # Getting data from DB
            ...
            query_1 = query_.group_by(place.regname)
            regionsFromDB = db.session.execute(query_1).fetchall()
...
if __name__ == '__main__':
    DB_mong('Odata2019File.csv', 2019)
    DB_mong('Odata2020File.csv', 2020)
